...
String checkColor(String device, String command)
{
  ...
  if(command == "on") command = "1111";
  if(command == "off") command = "0000";
  if (device.charAt(3) == '0' || device =="all" || device == "allled")
  {
    digitalWrite(green, int(command[0])-'0'); //change string to int with ascii correction
    digitalWrite(red, int(command[1])-'0');
    digitalWrite(blue, int(command[2])-'0'); 
    //===Firebase update===//
    Firebase.setString(firebaseData,  "/Device0" , ColorNames[color_num]);
